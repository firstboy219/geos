...
_SYSTEM = (
    "Anda analis dampak. Diberikan sebuah SITUASI geopolitik beserta skenario "
    "dan beritanya, jabarkan DAMPAK UMUM yang luas dan konkret bagi masyarakat "
    "dan ekonomi (contoh: ekspor-impor turun, harga BBM retail naik, sekolah "
    "tutup, pariwisata anjlok) DAN dampak ke kategori aset. Balas HANYA JSON:\n"
    ...
    "Berikan 8-14 dampak; usahakan mencakup kategori 'general' dan beberapa "
    "kelas aset (saham/emas/forex/crypto/properti). Bahasa Indonesia."
)
...
async def _build_user(db, crisis: Crisis) -> str:
    scen = (
        await db.execute(
            select(Scenario.name, Scenario.probability, Scenario.narrative_text)
    ...
    payload = {
        ...
        "scenarios": [
            {"name": n, "probability": p, "narrative": (t or "")[:300]}
            for n, p, t in scen
        ],
        "headlines": list(news),
    }
...
async def generate_for_crisis(db, crisis_id, *, force: bool = False) -> dict:
    ...
    try:
        data = await generate_json(_SYSTEM, await _build_user(db, crisis), temperature=0.5)
    except Exception as exc:  # noqa: BLE001
        logger.warning("impact gen failed for %s: %s", crisis.id, exc)
    ...
    await db.commit()
    result = {"status": "ok", "crisis_id": str(crisis.id), "impacts": n}
